                        ...
                        <div class="mb-4 flex flex-wrap">
                            <div class="w-1/2 pr-2">
                                ...
                                <select name="user_type" id="user_type"
                                    ...
                                    <option value="Regular" {{ $user->user_type === 'Regular' ? 'selected' : '' }}>
                                        Regular</option>
                                </select>
                                @error('user_type')
                                    <span class="text-red-600">{{ $message }}</span>
                                ...
                                <select name="company_id" id="company_id"
                                    ...
                                    @foreach ($companies as $company)
                                        <option value="{{ $company->company_id }}"
                                            {{ $user->company_id === $company->company_id ? 'selected' : '' }}>
                                            {{ $company->name }}</option>
                                    @endforeach
                                </select>
                                @error('company_id')
                        ...
                        <div class="mb-4">
                            ...
                            <div class="space-y-2">
                                @foreach ($menuLinks as $menuLink)
                                    @php
                                        $menuPermittedArray = explode(',', $user->menu_permitted);
                                    @endphp
                                    <label class="inline-flex items-center pr-1 bg-gray-500">
                                        <input type="checkbox" name="menu_permitted[]" value="{{ $menuLink->menu_id }}"
                                            ...
                                            {{ in_array($menuLink->menu_id, $menuPermittedArray) ? 'checked' : '' }}>
